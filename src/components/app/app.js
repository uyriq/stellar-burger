...
const App = () => {
    ...
    const dispatchdata = useCallback(() => {
        if (undefined !== data && null !== data && !Boolean(dataState?.data.length)) {
    ...
    return (
        <div className={Styles.page}>
            {isError && <p className={`${Styles.page} text_color_error `}>
                Что-то пошло не так, не получены данные, {isError} </p>}
